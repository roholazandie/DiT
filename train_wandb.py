...
def _safe_getexif(self):
    try:
        ...
        # if anything goes wrong parsing EXIF, just return empty dict
        return {}
...
def main(args):
    ...
    if rank == 0:
        ...
        wandb.init(
            ...
            config=vars(args),
            name=f"run-{experiment_index:03d}-{model_name}"
        )
    else:
    ...
    base_model = DiT_models[args.model](input_size=latent_size,
                                        num_classes=args.num_classes)
    ema_model = deepcopy(base_model).to(device)
    requires_grad(ema_model, False)
    model = DDP(base_model.to(device), device_ids=[rank])
    ...
    logger.info(f"Dataset contains {len(dataset):,} training examples")

    # Training states
    update_ema(ema_model, model.module, decay=0)
    model.train()
    ema_model.eval()

    train_steps, log_steps, running_loss = 0, 0, 0.0
    start_time = time()
        ...
        for x, y in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0:
                    wandb.log({
                        "train/loss": avg_loss,
                        "train/log_loss": math.log(avg_loss),
            ...
            if train_steps % 1000 == 0 and rank == 0:
                ...
                wandb.log({"samples": [wandb.Image(grid)]}, step=train_steps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb-project", type=str, default="DiT-training",
